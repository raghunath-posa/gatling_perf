...
class Dogs extends Simulation {

  val httpConfDogs = http
  ...
  val getDogsScenario = scenario("Dogs")
    .exec(http("Get Dogs")
      .get("/animals/api/dogs")
...
//  ).protocols(httpConfDogs)
  ...
  setUp(
      getDogsScenario.inject(rampUsers(30) over 1.seconds).protocols(httpConfDogs)).throttle(reachRps(10) in 10.seconds , holdFor(1 minutes)
  )
}
